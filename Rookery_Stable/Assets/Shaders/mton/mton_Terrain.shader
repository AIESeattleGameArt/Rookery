Shader "MTON/layeredTerrain" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			half4 base = tex2D (_MainTex, IN.uv_MainTex * float2(_tileMain, _tileMain)) * mask.b;
			half4 detail = tex2D (_DetailTex, IN.uv_MainTex * float2(_tileDetail, _tileDetail)) * (1.0-mask.b);
			half4 layer1 = (base+detail) * mask.r;
			half4 wall = tex2D (_WallTex, IN.uv_MainTex * float2(_tileWall, _tileWall)) * (1.0-mask.r);
			half4 c = (layer1 + wall) * (mask.g + _ColorCrevice.a) * half4(_ColorCrevice.rgb, 1.0); 
			o.Albedo = c.rgb;
			o.Alpha = c.a;
		}
		ENDCG
